...
const Home = () => {
  ...
  const { loading, error, restaurants, showVegOnly, creating, createError } =
  ...
  const [newRestaurant, setNewRestaurant] = useState({
    name: "", address: "", isVeg: false,
    location: { type: "Point", coordinates: [] }, imageUrl: "",
  });
  ...
  const handleChange = (e) => {
    ...
    if (name === "isVeg") {
      ...
      const coords = value.split(",").map((v) => parseFloat(v.trim())).filter((n) => !isNaN(n));
      setNewRestaurant({ ...newRestaurant, location: { ...newRestaurant.location, coordinates: coords } });
            ...
            {isAuthenticated && user && user.role === "admin" && (
              <div className="col-sm-12 col-md-6 col-lg-3 my-3">
                <div
                  ...
                  onClick={() => { setCoordsInput(""); setShowCreate(true); }}
                >
                  <h1 className="m-0">+</h1>
            ...
            <div className="create-modal">
              <div className="create-content">
                ...
                <form onSubmit={submitCreate}>
                  ...
                  <div className="form-group">
                    <label>Address</label>
                    <input type="text" name="address" value={newRestaurant.address} onChange={handleChange} required />
                  </div>
                  <div className="form-group">
                    <label>Pure Veg</label>
                    <input type="checkbox" name="isVeg" checked={newRestaurant.isVeg} onChange={handleChange} />
                  </div>
                  <div className="form-group">
                    <label>Coordinates (lat,lng)</label>
                    <input type="text" name="coordinates" value={coordsInput} onChange={handleChange} placeholder="e.g. 40.77,-73.97" required />
                  </div>
                  <div className="form-group">
                    <label>Image URL</label>
                    <input type="text" name="imageUrl" value={newRestaurant.imageUrl} onChange={handleChange} placeholder="https://..." required />
                  </div>
                  <button className="btn btn-primary" type="submit" disabled={creating}>
                    {creating ? "Creating..." : "Create"}
                  </button>
                  <button className="btn btn-secondary ml-2" type="button" onClick={() => setShowCreate(false)}>
                    Cancel
                  </button>
                </form>
              </div>
